...
namespace CAMASSEMBLYMACHINE.UI
{
    ...
    public partial class tabProdLoaderTeach : Form, IForm
    {
        public enum CAMPAGE
        {
            UNKNOWN = -1,
        ...
        public BTN_TAB_SELECT mLeftTeachingPoint = BTN_TAB_SELECT.XYRZ_LEFT_PLACE_LEFT;
        public BTN_TAB_SELECT mRightTeachingPoint = BTN_TAB_SELECT.XYRZ_RIGHT_PLACE_LEFT;
        public List<NV_UI.NV_Button_TG_NS> listTeachingPointBtn = new List<NV_UI.NV_Button_TG_NS>();
        tabProdLoaderPickerCalib tabProdLoaderPickerCalib; //tabProdLoaderPickerCalib;
        double maxX, maxY, maxR1, maxR2, maxZ;
        double minX, minY, minR1, minR2, minZ;
        RecipeDefine.POSITION curX, curY, curR1, curR2, curZ;
        public tabProdLoaderTeach()
        {
            InitializeComponent();
            InitializeVariable();
        ...
        private void InitializeVariable()
        {
            ...
            this.mCurrentTeachingPoint = BTN_TAB_SELECT.XYRZ_LEFT_PLACE_LEFT;
            ...
            tabProdLoaderPickerCalib = new tabProdLoaderPickerCalib((int)CAMERA.PICKER);
            tabProdLoaderPickerCalib.TopLevel = false;
            tabProdLoaderPickerCalib.Parent = panelUnder;
            tabProdLoaderPickerCalib.Dock = DockStyle.Fill;
            tabProdLoaderPickerCalib.Show();
        ...
        public void StartTimer(bool enable)
        {
            ...
            {
                timer1.Start();
                if (curPage == CAMPAGE.VISION_PICKER)
                    tabProdLoaderPickerCalib.StartTimer(true);
                UpdateTeachingValue();
            }
            else
            {
                timer1.Stop();
                tabProdLoaderPickerCalib.StartTimer(false);
            }
        }
        ...
        private void UpdateServoState()
        {
            ...
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_X, ref ServoStatusCheck);
                if (!ServoStatusCheck)
                    ServoStatus = false;
                Machine.motion.GetAlarm((int)AXIS.PROD_PICKUP_X, ref AlarmStatusCheck);
                if (AlarmStatusCheck)
                    AlarmStatus = true;
            }
            if (teachingInputValue2.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_Y, ref ServoStatusCheck);
                if (!ServoStatusCheck)
                    ServoStatus = false;
                Machine.motion.GetAlarm((int)AXIS.PROD_PICKUP_Y, ref AlarmStatusCheck);
                if (AlarmStatusCheck)
                    AlarmStatus = true;
            }
            if (teachingInputValue3.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_R1, ref ServoStatusCheck);
                if (!ServoStatusCheck)
                    ServoStatus = false;
                Machine.motion.GetAlarm((int)AXIS.PROD_PICKUP_R1, ref AlarmStatusCheck);
                if (AlarmStatusCheck)
                    AlarmStatus = true;
            }
            if (teachingInputValue4.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_R2, ref ServoStatusCheck);
                if (!ServoStatusCheck)
                    ServoStatus = false;
                Machine.motion.GetAlarm((int)AXIS.PROD_PICKUP_R2, ref AlarmStatusCheck);
                if (AlarmStatusCheck)
                    AlarmStatus = true;
            }
            if (teachingInputValue5.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_Z, ref ServoStatusCheck);
                if (!ServoStatusCheck)
                    ServoStatus = false;
                Machine.motion.GetAlarm((int)AXIS.PROD_PICKUP_Z, ref AlarmStatus);
                if (AlarmStatusCheck)
                    AlarmStatus = true;
            }
        }
        ...
        private void UpdateAxisState()
        {
            ...
            Machine.motion.GetAxisActualPos((int)AXIS.PROD_PICKUP_Z, ref actualPos);
            teachingInputValue5.CurrentPos = actualPos / 1000.0;

            Machine.motion.GetAxis_POSLimit((int)AXIS.PROD_PICKUP_X, ref isHW_POSLimit);
            Machine.motion.GetAxis_NEGLimit((int)AXIS.PROD_PICKUP_X, ref isHW_NEGLimit);
            Machine.motion.GetAxisHomeComplete((int)AXIS.PROD_PICKUP_X, ref isHW_POSHome);
            ...
            Machine.motion.GetAxis_POSLimit((int)AXIS.PROD_PICKUP_Y, ref isHW_POSLimit);
            Machine.motion.GetAxis_NEGLimit((int)AXIS.PROD_PICKUP_Y, ref isHW_NEGLimit);
            Machine.motion.GetAxisHomeComplete((int)AXIS.PROD_PICKUP_Y, ref isHW_POSHome);
            ...
            Machine.motion.GetAxis_POSLimit((int)AXIS.PROD_PICKUP_R1, ref isHW_POSLimit);
            Machine.motion.GetAxis_NEGLimit((int)AXIS.PROD_PICKUP_R1, ref isHW_NEGLimit);
            Machine.motion.GetAxisHomeComplete((int)AXIS.PROD_PICKUP_R1, ref isHW_POSHome);
            ...
            Machine.motion.GetAxis_POSLimit((int)AXIS.PROD_PICKUP_R2, ref isHW_POSLimit);
            Machine.motion.GetAxis_NEGLimit((int)AXIS.PROD_PICKUP_R2, ref isHW_NEGLimit);
            Machine.motion.GetAxisHomeComplete((int)AXIS.PROD_PICKUP_R2, ref isHW_POSHome);
            ...
            Machine.motion.GetAxis_POSLimit((int)AXIS.PROD_PICKUP_Z, ref isHW_POSLimit);
            Machine.motion.GetAxis_NEGLimit((int)AXIS.PROD_PICKUP_Z, ref isHW_NEGLimit);
            Machine.motion.GetAxisHomeComplete((int)AXIS.PROD_PICKUP_Z, ref isHW_POSHome);
        ...
        {
            XYRZ_LEFT_PLACE_LEFT,
            XYRZ_RIGHT_PLACE_LEFT,

            XYRZ_LEFT_PLACE_RIGHT,
            XYRZ_RIGHT_PLACE_RIGHT,
            ...
            if (Enum.TryParse(sTag, out BTN_TAB_SELECT selectBtn))
            {
                ...
                switch (selectBtn)
                {
                    case BTN_TAB_SELECT.XYRZ_LEFT_PICKUP:
                        //curX = RecipeDefine.POSITION.PROD_LOADER_TRAY_CEN1_X;
                        //curY = RecipeDefine.POSITION.PROD_LOADER_TRAY_CEN1_Y;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PICKUP_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PICKUP_POS;
                    ...
                    case BTN_TAB_SELECT.XYRZ_RIGHT_PICKUP:
                        //curX = RecipeDefine.POSITION.PROD_LOADER_TRAY_CEN2_X;
                        //curY = RecipeDefine.POSITION.PROD_LOADER_TRAY_CEN2_Y;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PICKUP_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PICKUP_POS;
                        ...
                        button_ZMinus.Enabled = true;                        button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_LEFT_PLACE_LEFT:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_BUF_L_L_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_BUF_L_L_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PLACE_L_L_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PLACE_POS;
                        ...
                        button_ZMinus.Enabled = true;                        button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_LEFT_PLACE_RIGHT:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_BUF_L_R_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_BUF_L_R_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PLACE_L_R_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PLACE_POS;
                        ...
                        button_ZMinus.Enabled = true; button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_RIGHT_PLACE_LEFT:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_BUF_R_L_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_BUF_R_L_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PLACE_R_L_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PLACE_POS;
                        ...
                        button_ZMinus.Enabled = true;                        button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_RIGHT_PLACE_RIGHT:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_BUF_R_R_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_BUF_R_R_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_PLACE_R_R_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_PLACE_POS;
                        ...
                        button_ZMinus.Enabled = true; button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_ALIGN_LL:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_ALIGN_L_L_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_ALIGN_L_L_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_ALIGN_L_L_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_ALIGN_POS;
                        ...
                        button_ZMinus.Enabled = true; button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_ALIGN_LR:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_ALIGN_L_R_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_ALIGN_L_R_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_ALIGN_L_R_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_ALIGN_POS;
                    ...
                    case BTN_TAB_SELECT.XYRZ_ALIGN_RL:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_ALIGN_R_L_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_ALIGN_R_L_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_ALIGN_R_L_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_ALIGN_POS;
                        ...
                        button_ZMinus.Enabled = true; button_ZPlus.Enabled = true;
                        break;
                    case BTN_TAB_SELECT.XYRZ_ALIGN_RR:
                        curX = RecipeDefine.POSITION.PROD_LOADER_X_ALIGN_R_R_POS;
                        curY = RecipeDefine.POSITION.PROD_LOADER_Y_ALIGN_R_R_POS;
                        curR2 = curR1 = RecipeDefine.POSITION.PROD_LOADER_R_ALIGN_R_R_POS;
                        curZ = RecipeDefine.POSITION.PROD_LOADER_Z_ALIGN_POS;
        ...
        private void Tab_Select_First_Shown(object sender, EventArgs e)
        {
            if (firstStart)
            {
                SelectButton_ClickEvent(nV_Button_TG_NS_XYRZLeftPlaceLeft, null);
                firstStart = false;
            }
        }
        #endregion
        ...
        {           
            nV_Button_TG_NS_XYRZLeftPlaceLeft.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZRightPlaceLeft.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZLeftPlaceRight.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZRightPlaceRight.ClickEvent += SelectButton_ClickEvent;
            ...
            nV_Button_TG_NS_XYRZAlignLL.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZAlignLR.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZAlignRL.ClickEvent += SelectButton_ClickEvent;
            nV_Button_TG_NS_XYRZAlignRR.ClickEvent += SelectButton_ClickEvent;

            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZLeftPlaceLeft);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZRightPlaceLeft);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZLeftPlaceRight);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZRightPlaceRight);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZLeftPickUp);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZRightPickUp);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZAlignLL);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZAlignLR);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZAlignRL);
            listTeachingPointBtn.Add(nV_Button_TG_NS_XYRZAlignRR);
        ...
        private void FunctionBtnClickEvent(object sender, EventArgs e)
        {
            ...
            switch (eBtnType)
            {
                case TEACHING_TASK.MOVE_TARGET:
                    LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Move Target Button Click", CONTENT_TYPE.INFO);
        ...
        private async void MoveTarget(AXIS_DIRECTION2 Pos)
        {
            ...
            int iAxisdx = 0;
            int iAxisdx2 = 0;
            double vel_x = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_X_MOVE_VEL);
            double acc_x = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_X_MOVE_ACC);
            double dec_x = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_X_MOVE_DEC);
            double vel_y = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Y_MOVE_VEL);
            double acc_y = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Y_MOVE_ACC);
            double dec_y = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Y_MOVE_DEC);
            double vel_r = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_R_MOVE_VEL);
            double acc_r = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_R_MOVE_ACC);
            double dec_r = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_R_MOVE_DEC);
            double vel_z = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Z_MOVE_VEL);
            double acc_z = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Z_MOVE_ACC);
            double dec_z = 1000 * Machine.param.Velocity(ParameterDefine.VELOCITY.PROD_LOADER_Z_MOVE_DEC);
            double pos = 0.0;
            ...
            action = new Action(delegate
            {
                switch (Pos)
                {
                    case AXIS_DIRECTION2.X:
                        iAxisdx = (int)AXIS.PROD_PICKUP_X;
                    ...
                    case AXIS_DIRECTION2.Y:
                        iAxisdx = (int)AXIS.PROD_PICKUP_Y;
                    ...
                    case AXIS_DIRECTION2.R1:
                        iAxisdx = (int)AXIS.PROD_PICKUP_R1;
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_R1, true);
        }
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_R1, false);
        }
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_R2, true);
        }
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_R2, false);
        }
        ...
        private void button_YPlus_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            MoveJogAxis((int)AXIS.PROD_PICKUP_Y, true);
        }
        ...
        private void button_YMinus_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            MoveJogAxis((int)AXIS.PROD_PICKUP_Y, false);
        }
        ...
        {
            Machine.interfer_release_x_safe_area = true;
            MoveJogAxis((int)AXIS.PROD_PICKUP_X, true);
        }
        ...
        {
            Machine.interfer_release_x_safe_area = true;
            MoveJogAxis((int)AXIS.PROD_PICKUP_X, false);
        }
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_Z, true);
        }
        ...
        {
            MoveJogAxis((int)AXIS.PROD_PICKUP_Z, false);
        }
        ...
        private void button_ServoOnOff_Click(object sender, EventArgs e)
        {
            ...
            if (teachingInputValue1.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_X, ref servoStatus);
                if (servoStatus)
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_X, 0);
                else
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_X, 1);
            }
            if (teachingInputValue2.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_Y, ref servoStatus);
                if (servoStatus)
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_Y, 0);
                else
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_Y, 1);
            }
            if (teachingInputValue3.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_R1, ref servoStatus);
                if (servoStatus)
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_R1, 0);
                else
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_R1, 1);
            }
            if (teachingInputValue4.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_R2, ref servoStatus);
                if (servoStatus)
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_R2, 0);
                else
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_R2, 1);
            }       
            if (teachingInputValue5.Clicked)
            {
                Machine.motion.GetAxisServoOn((int)AXIS.PROD_PICKUP_Z, ref servoStatus);
                if (servoStatus)
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_Z, 0);
                else
                    Machine.motion.SetAxisServoOn((int)AXIS.PROD_PICKUP_Z, 1);
            }
        }

        private void button_AlarmReset_Click(object sender, EventArgs e)
        {
            for (int i = (int)AXIS.PROD_PICKUP_X; i <= (int)AXIS.PROD_PICKUP_R2; ++i)
            {
        ...
        private void tabControl2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (tabControl2.SelectedIndex == 1) //Vision
            {
                ...
                {
                    curPage = CAMPAGE.VISION_PICKER;
                    tabProdLoaderPickerCalib.StartTimer(true);
                    tabProdLoaderPickerCalib.Show();
                    tabProdLoaderPickerCalib.UpdateLightSetting();
                }
            }
        }
        private void tbxSpeedValue1_DoubleClick(object sender, EventArgs e)
        {
            ...
            SubForm_TenKey keyPad = new SubForm_TenKey(ref value, "XY Vel Set", "Vel Set", 0, 100000, 300, 300, true);
            keyPad.ShowDialog();
        ...
        private void tbxSpeedValue2_DoubleClick(object sender, EventArgs e)
        {
            ...
            SubForm_TenKey keyPad = new SubForm_TenKey(ref value, "RZ Vel Set", "Vel Set", 0, 100000, 300, 300, true);
            keyPad.ShowDialog();
