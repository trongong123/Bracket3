...
{
    public partial class tabProdLoaderTrayCalib : Form, IForm
    {
        #region VARIABLES
        CALIBRATION[] eParam =
        {
            CALIBRATION.TRAY_CAL_MAP_START_POS_X,
            CALIBRATION.TRAY_CAL_MAP_START_POS_Y,
            CALIBRATION.TRAY_CAL_MAP_END_POS_X,
            CALIBRATION.TRAY_CAL_MAP_END_POS_Y,
            CALIBRATION.TRAY_CAL_MAP_POS_Z,
            CALIBRATION.TRAY_CAL_MAP_STEP,
        };
        string[] tags;
        ...
        #region INITIALIZE
        public tabProdLoaderTrayCalib(int Id)
        {
            InitializeComponent();
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Tray Vpp Edit Button Click", CONTENT_TYPE.INFO);
        ...
        private void btnMappingCalibration_Click(object sender, EventArgs e)
        {
            ...
            calMapForm = new tabCalMapControl(CAMERA.TRAY);
            calMapForm.ShowDialog();
        }

        private void btnMappingSeqRight_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Tray Mapping Right Calibration Button Click", CONTENT_TYPE.INFO);
        ...
        private void btnMappingSeqDual_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Tray Mapping Right Calibration Button Click", CONTENT_TYPE.INFO);
        ...
        private void UpdateAxisState()
        {
            ledMapStartXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.TRAY_CAL_MAP_START_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.TRAY_CAL_MAP_START_POS_Y));

            ledMapEndXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.TRAY_CAL_MAP_END_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.TRAY_CAL_MAP_END_POS_Y));
            ...
            ledZCalPos.ButtonPush = Util.IsAxisInPos(
                (AXIS.PROD_PICKUP_Z, CALIBRATION.TRAY_CAL_MAP_POS_Z));
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Move to Tray Calibration 1 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.PROD_LOADER];
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_MOVE_MAP_TRAY_START_XY);
        }

        private void button_MapEndXY_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Move to Tray Calibration 2 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.PROD_LOADER];
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_MOVE_MAP_TRAY_END_XY);
        }
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "ProdLoader Move to Tray Calibration (Z) Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.PROD_LOADER];
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_SINGLE, (int)ProcessProdLoader.STEP.MOVE_Z_TRAY_CAL_POS);
        }
        ...
        private void dataGridViewCamInfo_VisibleChanged(object sender, EventArgs e)
        {
            ...
            tbxRatioXLeft.Text = Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioYLeft.Text = Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioXRight.Text = Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT].ToString();
            tbxRatioYRight.Text = Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT].ToString();
        }
        ...
        private void UpdateCameraInfo(DataGridView dtgvCamInfo, TOOL_TYPE toolType)
        {
            try
            {
                ...
                dtgvCamInfo.Rows[0].Cells[1].Value = Vision.GetCalMapData(CAMERA.TRAY, toolType).Count();
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[1].Cells[0].Value = $"Angle";
                dtgvCamInfo.Rows[1].Cells[1].Value = Math.Round(Vision.GetCameraAngle(CAMERA.TRAY, toolType), 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[2].Cells[0].Value = $"Avg Pixel Size X";
                dtgvCamInfo.Rows[2].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.TRAY, (int)toolType].sizeX, 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[3].Cells[0].Value = $"Avg Pixel Size Y";
                dtgvCamInfo.Rows[3].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.TRAY, (int)toolType].sizeY, 4);
        ...
        private void tbxRatio_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tag == "LEFT_X") Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "LEFT_Y") Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "RIGHT_X") Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                if (tag == "RIGHT_Y") Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                Vision.Camera.RatioLeftX((int)CAMERA.TRAY, Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioLeftY((int)CAMERA.TRAY, Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioRightX((int)CAMERA.TRAY, Vision.calibMapRatioX[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT]);
                Vision.Camera.RatioRightY((int)CAMERA.TRAY, Vision.calibMapRatioY[(int)CAMERA.TRAY, (int)TOOL_TYPE.RIGHT]);
                Vision.UpdateCalibListByRatio(CAMERA.TRAY, TOOL_TYPE.LEFT);
                Vision.UpdateCalibListByRatio(CAMERA.TRAY, TOOL_TYPE.RIGHT);
                Vision.Camera.Write();
            }
            catch (Exception EX)
            {
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.DATA, $"ProdLoader Tray Calibration / LightSet Save Button Click", CONTENT_TYPE.INFO);
        ...
        private void button_Test_Click(object sender, EventArgs e)
        {
            ...
            proc.SetMessage((int)ProcessTrayWork.MSG.MSG_SINGLE, (int)ProcessTrayWork.STEP.REQUEST_VISIOTN_ALIGN);
        }
