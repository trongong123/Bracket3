...
{
    public class ProcessTrayIn : IProcess
    {
        ...
        public enum STEP
        {
            ...
            LIGHT_CURTAIN_MUTING_ON,
            LIGHT_CURTAIN_MUTING_OFF,
        }
        ...
        private void SetError(ECODE error)
        {
            ...
            switch ((ECODE)error)
            {
                ...
                case ECODE.MISMATCH_BETWEEN_DATA_AND_PRODUCT_TRAY_IN:
        ...
        protected override void OnProcessing()
        {
            switch ((STEP)Step)
            {
                ...
                case STEP.INIT_PRODUCT_DATA_MISMATCH_CHECK:
                    ...
                    {
                        Machine.Trays[(int)UNITTRAY.TRAY_IN].exist = Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.END);
                    }
                ...
                case STEP.CONV_STOP_WITH_DELAY_CHECK:
                    Util.Delay((int)Machine.param.Time(ParameterDefine.TIME.TRAY_CONV_STOP_DELAY_TIME));
                    Step = STEP.CONV_STOP;
                    break;

                case STEP.INIT_STOPPER_UP:
                    if (!Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.END)
                        && !Machine.GetTraySensorSignal(TRAY_TYPE.IN_BUF, TRAY_CV_SENSOR.IN))
        ...
        private void OnProcessOfAutoRun()
        {
            switch ((AUTOSTEP)AutoStep)
            {
                ...
                case AUTOSTEP.PROC_SELECT_FOR_RESTART:
                    if (CheckStopBit())
                        break;
                    if (Machine.Trays[(int)UNITTRAY.TRAY_IN].exist)
        ...
        {
            int current = (int)UNITTRAY.TRAY_IN;
            int downstream = (int)UNITTRAY.TRAY_IN_BUF;
            ...
            switch ((STEP)Step)
            {
                case STEP.IF_LOADING_POSSIBLE_WAIT:
                    ...
                    if (Machine.AloneMode) NextStep();

                    if (Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.IN)
                        || Machine.GetTraySensorSignal(TRAY_TYPE.IN,TRAY_CV_SENSOR.MID))
                        NextStep();
                    timeWait[(int)TIMER.TIMEOUT].Reset();
                    break;
                ...
                case STEP.IF_LOADING_COMPLETE_CHECK:
                    ...
                    {
                        SetError(ECODE.TIMEOUT_TRAY_IN_LOADING);
                        break;
                    }
                    ...
                    {
                        if (!Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.END)) break;
                    }
                ...
                case STEP.IF_UNLOADING_COMPLETE_CHECK:
                    ...
                    if (timeWait[(int)TIMER.TIMEOUT].Elapsed > Machine.param.Time(ParameterDefine.TIME.CONV_RUN_TIME_OUT))
                    {
                        SetError(ECODE.TIMEOUT_TRAY_IN_UNLOADING);
                        break;
                    }
                    ...
                    else
                    {
                        if (Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.IN)
                                || Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.MID)
                                || Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.END)) break;
                    }
                ...
                case STEP.IF_REVERSE_UNLOADING_COMPLETE_CHECK:
                    bool ret = Machine.GetTraySensorSignal(TRAY_TYPE.IN, TRAY_CV_SENSOR.IN);
                    // 더미런 시에는 항상 ON 이다.
        ...
        private void OnProcessOfIO()
        {
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.STOPPER_UP_CHECK:
                    ...
                    {
                        SetError(ECODE.TIMEOUT_TRAY_IN_STOPPER_UP);
                        break;
                    }
                 
                    Machine.IO.GetIn((int)DI.TRAY_IN_STOPPER_UP, ref ret1);
                    Machine.IO.GetIn((int)DI.TRAY_IN_STOPPER_DOWN, ref ret2);
                ...
                case STEP.STOPPER_DOWN_CHECK:
                    if (timeWait[(int)TIMER.TIMEOUT].Elapsed > Machine.param.Time(ParameterDefine.TIME.CYLINDER_TIME_OUT))
                    {
                        SetError(ECODE.TIMEOUT_TRAY_IN_STOPPER_DOWN);
                        break;
                    }
                ...
                case STEP.LIGHT_CURTAIN_MUTING_ON:
                    Machine.IO.SetOut((int)DO.LIGHT_CURTAIN_MUTING_IN, 1);
                    NextStep();
                    break;

                case STEP.LIGHT_CURTAIN_MUTING_OFF:
                    Machine.IO.SetOut((int)DO.LIGHT_CURTAIN_MUTING_IN, 0);
                    NextStep();
                    break;
            }
        }
