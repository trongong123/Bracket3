...
{
    public class ProcessJigInBuf : IProcess
    {
        ...
        public enum STEP
        {
            ...
            // INTERFACE
            IF_UPSTREAM_PRODUCT_EXSIT_CHECK,
        ...
        public override void EStop(bool pause)
        {
            ...
            Machine.Jigs[(int)UNITJIG.JIG_IN_BUF].ClearInterface();
            Machine.IO.SetOut((int)DO.JIG_IN_BUF_CONV_RUN, 0);
            Machine.IO.SetOut((int)DO.IF_MACHINE_RUN_SIG_FOR_FRONT, 0);
            Machine.IO.SetOut((int)DO.IF_JIG_LD_CONV_RUN_WORK, 0);
        }
        ...
        public override void SetMessage(int message, int step = 0)
        {
            ...
            switch ((MSG)message)
            {
                ...
                case MSG.MSG_LOADING_PRODUCT:
                    ...
                    {
                        STEP.IF_UPSTREAM_PRODUCT_EXSIT_CHECK,
                        STEP.STOPPER_UP,
                        STEP.CONV_RUN,
                        STEP.IF_CONV_RUN_ON,
                    ...
                    if (ProductExistCheck() == CONVSTATUS.LOADING)
                    {
                        ProcessLoadingProduct[GetStepIndex(ProcessLoadingProduct, STEP.IF_UPSTREAM_PRODUCT_EXSIT_CHECK)] = STEP.NEXT_STEP;
                    }
        ...
        protected override void OnProcessing()
        {
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.INIT_PRODUCT_DATA_MISMATCH_CHECK:
                    ...
                    {
                        Machine.Jigs[(int)UNITJIG.JIG_IN_BUF].exist =
                            Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.IN)
                            && Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.OUT);
                    }
        ...
        private void OnProcessOfAutoRun()
        {
            ...
            switch ((AUTOSTEP)AutoStep)
            {
                ...
                case AUTOSTEP.PROC_SELECT_FOR_RESTART:
                    if (CheckStopBit())
                        break;
                    Machine.IO.SetOut((int)DO.IF_MACHINE_RUN_SIG_FOR_FRONT, 1);
        ...
        private void OnProcessOfInterface()
        {
            ...
            const int CURRENT = (int)UNITJIG.JIG_IN_BUF;
            const int DOWNSTREAM = (int)UNITJIG.JIG_WORK;

            switch ((STEP)Step)
            {
                #region LOADING INTERFACE
                case STEP.IF_UPSTREAM_PRODUCT_EXSIT_CHECK:
                    resumeStepIndex = StepIndex;

                    // 상부에 자재가 있는지 확인 한다.
                    // alone mode & dry run 일 때 자재 정보를 자동으로 생성한다. (complete 될 수 있도록 넘긴다)
                    if (Machine.AloneMode)
                        NextStep();
                    else
                    {
                        // 상부 자재의 상태를 확인한다.
                        Machine.IO.GetIn((int)DI.IF_JIG_UPSTREAM_EXIST_CHECK_WORK, ref returnExist);
                        // 더미런 시에는 항상 ON 이다.
                        if (Machine.status.mode == SystemMode.SystemModeDRYRUN || resumeProc)
                            returnExist = 1;
                ...
                case STEP.IF_LOADING_COMPLETE_CHECK:
                    bool isJigExist = Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.IN) && Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.OUT);
                ...
                case STEP.IF_CONV_RUN_ON:
                    ...
                    {
                        Machine.IO.SetOut((int)DO.IF_JIG_LD_CONV_RUN_WORK, 1);
                        NextStep();
                    }
                ...
                case STEP.IF_UNLOADING_COMPLETE_CHECK:
                    isJigExist = Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.IN) || Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.OUT);
                    // 더미런 시에는 항상 OFF 이다.
        ...
        private void OnProcessOfMotion()
        {
            switch ((STEP)Step)
            {
                case STEP.INIT_STOPPER_UP:
                    if ((Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.OUT)
                        || Machine.GetJigSignal(JIG_TYPE.TOP_WORK, JIG_SENSOR.IN))
                       && !Machine.GetJigSignal(JIG_TYPE.TOP_IN, JIG_SENSOR.IN)) NextStep();
        ...
        private void OnProcessOfIO()
        {
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.STOPPER_UP_CHECK:
                    Machine.IO.GetIn((int)DI.JIG_IN_BUF_STOPPER_UP, ref ret1);
                    Machine.IO.GetIn((int)DI.JIG_IN_BUF_STOPPER_DOWN, ref ret2);
        ...
        private CONVSTATUS ProductExistCheck()
        {
            ...
            Machine.IO.GetIn((int)DI.JIG_IN_BUF_DETECT_IN, ref existIn);
            Machine.IO.GetIn((int)DI.JIG_IN_BUF_DETECT_OUT, ref existOut);
            Machine.IO.GetIn((int)DI.JIG_IN_BUF_STOPPER_DOWN, ref stopperDown);

            if (existIn == 1 && existOut == 1)
                return CONVSTATUS.EXIST;
            else if (existIn == 0 && existOut == 0)
                return CONVSTATUS.EMPTY;
            else if (stopperDown == 1)
                return CONVSTATUS.UNLOADING;
            else if (stopperDown == 0)
                return CONVSTATUS.LOADING;
