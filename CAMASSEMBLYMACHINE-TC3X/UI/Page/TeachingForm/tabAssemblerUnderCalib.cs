...
{
    public partial class tabAssemblerUnderCalib : Form, IForm
    {
        #region VARIABLES
        CALIBRATION[] eParam =
        {
            CALIBRATION.UNDER_LEFT_CAL_MAP_START_POS_X,
            CALIBRATION.UNDER_LEFT_CAL_MAP_START_POS_Y,
            CALIBRATION.UNDER_LEFT_CAL_MAP_END_POS_X,
            CALIBRATION.UNDER_LEFT_CAL_MAP_END_POS_Y,
            CALIBRATION.UNDER_RIGHT_CAL_MAP_START_POS_X,
            CALIBRATION.UNDER_RIGHT_CAL_MAP_START_POS_Y,
            CALIBRATION.UNDER_RIGHT_CAL_MAP_END_POS_X,
            CALIBRATION.UNDER_RIGHT_CAL_MAP_END_POS_Y,
            CALIBRATION.UNDER_CAL_MAP_POS_Z,
            CALIBRATION.UNDER_CAL_MAP_STEP,
        };
        string[] tags;
        ...
        #region INITIALIZE
        public tabAssemblerUnderCalib(int Id)
        {
            InitializeComponent();
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "Assembler Under Vpp Edit Button Click", CONTENT_TYPE.INFO);
        ...
        private void btnMappingCalibration_Click(object sender, EventArgs e)
        {
            ...
            calMapForm = new tabCalMapControl(CAMERA.UNDER);
        ...
        {
            ledMapLeftStartXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.ASSEMBLER_X, CALIBRATION.UNDER_LEFT_CAL_MAP_START_POS_X),
                (AXIS.ASSEMBLER_Y, CALIBRATION.UNDER_LEFT_CAL_MAP_START_POS_Y));

            ledMapLeftEndXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.ASSEMBLER_X, CALIBRATION.UNDER_LEFT_CAL_MAP_END_POS_X),
                (AXIS.ASSEMBLER_Y, CALIBRATION.UNDER_LEFT_CAL_MAP_END_POS_Y));

            ledMapRightStartXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.ASSEMBLER_X, CALIBRATION.UNDER_RIGHT_CAL_MAP_START_POS_X),
                (AXIS.ASSEMBLER_Y, CALIBRATION.UNDER_RIGHT_CAL_MAP_START_POS_Y));

            ledMapRightEndXY.ButtonPush = Util.IsAxisInPos(
                (AXIS.ASSEMBLER_X, CALIBRATION.UNDER_RIGHT_CAL_MAP_END_POS_X),
                (AXIS.ASSEMBLER_Y, CALIBRATION.UNDER_RIGHT_CAL_MAP_END_POS_Y));
            ...
            ledZCalPos.ButtonPush = Util.IsAxisInPos(
                (AXIS.ASSEMBLER_Z, CALIBRATION.UNDER_CAL_MAP_POS_Z));
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "Assembler Move to Under Calibration 1 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            proc.SetMessage((int)ProcessAssembler.MSG.MSG_MOVE_MAP_UNDER_START_XY);
        }

        private void button_MapEndXY_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "Assembler Move to Under Calibration 2 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.ASSEMBLER];
            proc.SetHeadTarget(0);
            proc.SetMessage((int)ProcessAssembler.MSG.MSG_MOVE_MAP_UNDER_END_XY);
        }
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "Assembler Move to Under Calibration Center (Z) Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.ASSEMBLER];
            proc.SetMessage((int)ProcessAssembler.MSG.MSG_SINGLE, (int)ProcessAssembler.STEP.MOVE_Z_UNDER_CAL_POS);
        }
        ...
        private void dataGridViewCamInfo_VisibleChanged(object sender, EventArgs e)
        {
            ...
            tbxRatioXLeft.Text = Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioYLeft.Text = Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioXRight.Text = Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT].ToString();
            tbxRatioYRight.Text = Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT].ToString();
        }
        ...
        private void UpdateCameraInfo(DataGridView dtgvCamInfo, TOOL_TYPE toolType)
        {
            try
            {
                ...
                dtgvCamInfo.Rows[0].Cells[1].Value = Vision.GetCalMapData(CAMERA.UNDER, toolType).Count();
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[1].Cells[0].Value = $"Angle";
                dtgvCamInfo.Rows[1].Cells[1].Value = Math.Round(Vision.GetCameraAngle(CAMERA.UNDER, toolType), 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[2].Cells[0].Value = $"Avg Pixel Size X";
                dtgvCamInfo.Rows[2].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.UNDER, (int)toolType].sizeX, 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[3].Cells[0].Value = $"Avg Pixel Size Y";
                dtgvCamInfo.Rows[3].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.UNDER, (int)toolType].sizeY, 4);
        ...
        private void tbxRatio_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tag == "LEFT_X") Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "LEFT_Y") Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "RIGHT_X") Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                if (tag == "RIGHT_Y") Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                Vision.Camera.RatioLeftX((int)CAMERA.UNDER, Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioLeftY((int)CAMERA.UNDER, Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioRightX((int)CAMERA.UNDER, Vision.calibMapRatioX[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT]);
                Vision.Camera.RatioRightY((int)CAMERA.UNDER, Vision.calibMapRatioY[(int)CAMERA.UNDER, (int)TOOL_TYPE.RIGHT]);
                Vision.UpdateCalibListByRatio(CAMERA.UNDER, TOOL_TYPE.LEFT);
                Vision.UpdateCalibListByRatio(CAMERA.UNDER, TOOL_TYPE.RIGHT);
                Vision.Camera.Write();
            }
            catch (Exception EX)
            {
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.DATA, $"Assembler Under Calibration / LightSet Save Button Click", CONTENT_TYPE.INFO);
