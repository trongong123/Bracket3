...
{
    public partial class tabProdLoaderPickerCalib : Form, IForm
    {
        #region VARIABLES
        CALIBRATION[] eParam =
        {
            CALIBRATION.PICKER_LEFT_CAL_MAP_START_POS_X,
            CALIBRATION.PICKER_LEFT_CAL_MAP_START_POS_Y,
            CALIBRATION.PICKER_LEFT_CAL_MAP_END_POS_X,
            CALIBRATION.PICKER_LEFT_CAL_MAP_END_POS_Y,
            CALIBRATION.PICKER_RIGHT_CAL_MAP_START_POS_X,
            CALIBRATION.PICKER_RIGHT_CAL_MAP_START_POS_Y,
            CALIBRATION.PICKER_RIGHT_CAL_MAP_END_POS_X,
            CALIBRATION.PICKER_RIGHT_CAL_MAP_END_POS_Y,
            CALIBRATION.PICKER_CAL_MAP_POS_Z,
            CALIBRATION.PICKER_CAL_MAP_STEP,
        };
        string[] tags;
        AXIS[] axislist = { AXIS.PROD_PICKUP_X, AXIS.PROD_PICKUP_Y, AXIS.PROD_PICKUP_Z};
        UIJOGVELINFO veltype = UIJOGVELINFO.NORMAL;
        private int InspNo = -1;
        bool refreshData = true;
        private TOOL_TYPE headType;
        Form_PanePickerJog paneJog;
        tabCalMapControl calMapForm;
        #endregion

        #region INITIALIZE
        public tabProdLoaderPickerCalib(int Id)
        {
            InitializeComponent();
        ...
        private void tabVisionManuProp_Load(object sender, EventArgs e)
        {
            ...
            paneJog = new Form_PanePickerJog(0);
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "Picker Vpp Edit Button Click", CONTENT_TYPE.INFO);
        ...
        private void btnMappingCalibration_Click(object sender, EventArgs e)
        {
            ...
            calMapForm = new tabCalMapControl(CAMERA.PICKER);
        ...
        {
            ledMapLeftStartXY.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.PICKER_LEFT_CAL_MAP_START_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.PICKER_LEFT_CAL_MAP_START_POS_Y));

            ledMapLeftEndXY.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.PICKER_LEFT_CAL_MAP_END_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.PICKER_LEFT_CAL_MAP_END_POS_Y));

            ledMapRightStartXY.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.PICKER_RIGHT_CAL_MAP_START_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.PICKER_RIGHT_CAL_MAP_START_POS_Y));

            ledMapRightEndXY.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_X, CALIBRATION.PICKER_RIGHT_CAL_MAP_END_POS_X),
                (AXIS.PROD_PICKUP_Y, CALIBRATION.PICKER_RIGHT_CAL_MAP_END_POS_Y));

            ledZReadyPos.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_Z, POSITION.PROD_LOADER_Z_READY_POS));

            ledZCalPos.ButtonPush = IsAxisInPos(
                (AXIS.PROD_PICKUP_Z, CALIBRATION.PICKER_CAL_MAP_POS_Z));

        }

        private bool IsAxisInPos(params (AXIS axis, Enum pos)[] items)
        {
            double actualPos = 0;
            foreach (var (axis, posEnum) in items)
            {
                ...
                double paramPos =
                    ...
                    posEnum is POSITION p ? 1000 * Machine.param.Position(p) :
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Picker Calibration 1 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_MOVE_MAP_PICKER_START_XY);
        }

        private void button_MapEndXY_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Picker Calibration 2 Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.PROD_LOADER];
            proc.SetHeadTarget(0);
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_MOVE_MAP_PICKER_END_XY);
        }

        private void btnMapRightStartXY_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Picker Calibration 1 Pos Button Click", CONTENT_TYPE.INFO);
        ...
        private void btnMapRightEndXY_Click(object sender, EventArgs e)
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Picker Calibration 1 Pos Button Click", CONTENT_TYPE.INFO);
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Ready (Z) Pos Button Click", CONTENT_TYPE.INFO);
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Move to Picker Calibration Center (Z) Pos Button Click", CONTENT_TYPE.INFO);
            ...
            var proc = Machine.proclist[(int)Machine.PROCESS.PROD_LOADER];
            proc.SetMessage((int)ProcessProdLoader.MSG.MSG_SINGLE, (int)ProcessProdLoader.STEP.MOVE_Z_PICKER_CAL_POS);
        }
        ...
        private void button_ZCylDown_Click(object sender, EventArgs e)
        {
            ...
            if (ToolZUp == 1)
            {
                LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Left Picker Down Button Click", CONTENT_TYPE.INFO);
            ...
            {
                LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Left Picker Up Button Click", CONTENT_TYPE.INFO);
        ...
        private void button_Z2CylDown_Click(object sender, EventArgs e)
        {
            ...
            if (ToolZUp == 1)
            {
                LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Right Picker Down Button Click", CONTENT_TYPE.INFO);
            ...
            {
                LogUtil.Instance.Log(LOG_TYPE.UI, "PICKER Right Picker Up Button Click", CONTENT_TYPE.INFO);
        ...
        private void dataGridViewCamInfo_VisibleChanged(object sender, EventArgs e)
        {
            ...
            tbxRatioXLeft.Text = Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioYLeft.Text = Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT].ToString();
            tbxRatioXRight.Text = Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT].ToString();
            tbxRatioYRight.Text = Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT].ToString();
        }
        ...
        private void UpdateCameraInfo(DataGridView dtgvCamInfo, TOOL_TYPE toolType)
        {
            try
            {
                ...
                dtgvCamInfo.Rows[0].Cells[1].Value = Vision.GetCalMapData(CAMERA.PICKER, toolType).Count();
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[1].Cells[0].Value = $"Angle";
                dtgvCamInfo.Rows[1].Cells[1].Value = Math.Round(Vision.GetCameraAngle(CAMERA.PICKER, toolType), 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[2].Cells[0].Value = $"Avg Pixel Size X";
                dtgvCamInfo.Rows[2].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.PICKER, (int)toolType].sizeX, 4);
                dtgvCamInfo.Rows.Add();
                dtgvCamInfo.Rows[3].Cells[0].Value = $"Avg Pixel Size Y";
                dtgvCamInfo.Rows[3].Cells[1].Value = Math.Round(Vision.pixelSize[(int)CAMERA.PICKER, (int)toolType].sizeY, 4);
        ...
        private void tbxRatio_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tag == "LEFT_X") Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "LEFT_Y") Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT] = double.Parse(value);
                if (tag == "RIGHT_X") Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                if (tag == "RIGHT_Y") Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT] = double.Parse(value);
                Vision.Camera.RatioLeftX((int)CAMERA.PICKER, Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioLeftY((int)CAMERA.PICKER, Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.LEFT]);
                Vision.Camera.RatioRightX((int)CAMERA.PICKER, Vision.calibMapRatioX[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT]);
                Vision.Camera.RatioRightY((int)CAMERA.PICKER, Vision.calibMapRatioY[(int)CAMERA.PICKER, (int)TOOL_TYPE.RIGHT]);
                Vision.UpdateCalibListByRatio(CAMERA.PICKER, TOOL_TYPE.LEFT);
                Vision.UpdateCalibListByRatio(CAMERA.PICKER, TOOL_TYPE.RIGHT);
                Vision.Camera.Write();
            }
            catch (Exception EX)
            {
        ...
        {
            LogUtil.Instance.Log(LOG_TYPE.DATA, $"Picker Calibration / LightSet Save Button Click", CONTENT_TYPE.INFO);
