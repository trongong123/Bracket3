...
{
    public class ProcessJigInCarrier : IProcess
    {
        ...
        public override void EStop(bool pause)
        {
            ...
            Machine.Jigs[(int)UNITJIG.JIG_IN_CARRIER].ClearInterface();
            Machine.IO.SetOut((int)DO.JIG_IN_CARRIER_CONV_RUN, 0);
            Machine.IO.SetOut((int)DO.IF_JIG_LD_CONV_RUN_CARRIER, 0);
        }
        ...
        protected override void OnProcessing()
        {
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.INIT_PRODUCT_DATA_MISMATCH_CHECK:
                    ...
                    {
                        Machine.Jigs[(int)UNITJIG.JIG_IN_CARRIER].exist =
                            Machine.GetJigSignal(JIG_TYPE.UNDER_IN, JIG_SENSOR.IN)
        ...
        private void OnProcessOfInterface()
        {
            ...
            const int CURRENT = (int)UNITJIG.JIG_IN_CARRIER;
            const int DOWNSTREAM = (int)UNITJIG.JIG_OUT_CARRIER;
            bool signalOn = false;
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.IF_UPSTREAM_PRODUCT_EXSIT_CHECK:
                    ...
                    {
                        // 상부 자재의 상태를 확인한다.
                        // 조명에 의한 신호 일 수 있으니 1초 대기 한다.
                        Machine.IO.GetIn((int)DI.IF_JIG_UPSTREAM_EXIST_CHECK_CARRIER, ref returnExist);
                        // 더미런 시에는 항상 ON 이다.
                ...
                case STEP.IF_LOADING_COMPLETE_CHECK:
                    bool isJigArrival = Machine.GetJigSignal(JIG_TYPE.UNDER_IN, JIG_SENSOR.OUT);
                    // 더미런 시에는 항상 ON 이다.
                    if (Machine.status.mode == SystemMode.SystemModeDRYRUN)
                    {
                        isJigArrival = true;
                ...
                case STEP.IF_CONV_RUN_ON:
                    ...
                    {
                        Machine.IO.SetOut((int)DO.IF_JIG_LD_CONV_RUN_CARRIER, 1);
                        NextStep();
                    }
                ...
                case STEP.IF_UNLOADING_COMPLETE_CHECK:
                    bool isJigUnloaded = !Machine.GetJigSignal(JIG_TYPE.UNDER_IN, JIG_SENSOR.OUT)
                        && !Machine.GetJigSignal(JIG_TYPE.UNDER_IN, JIG_SENSOR.IN);
                    // 더미런 시에는 항상 OFF 이다.
                    if (Machine.status.mode == SystemMode.SystemModeDRYRUN) isJigUnloaded = true;

                    // 자재 감지 신호가 OFF 되면 확인한다.
                    if (!isJigUnloaded)
                        timeWait[(int)TIMER.TRIG].Reset();
                    else
                    {
                        // 자재가 배출되었다고 판단되면 데이터를 생성한다.
        ...
        private void OnProcessOfMotion()
        {
            switch ((STEP)Step)
            {
                case STEP.INIT_STOPPER_UP:
                    if (Machine.GetJigSignal(JIG_TYPE.UNDER_IN, JIG_SENSOR.OUT)
                        || Machine.GetJigSignal(JIG_TYPE.UNDER_OUT, JIG_SENSOR.IN)) NextStep();
        ...
        private void OnProcessOfIO()
        {
            ...
            switch ((STEP)Step)
            {
                ...
                case STEP.STOPPER_UP_CHECK:
                    ...
                    {
                        SetError(ECODE.TIMEOUT_JIG_INCARRIER_STOPPER_UP);
                        break;
                    }
                    Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_STOPPER_UP, ref ret1);
                    Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_STOPPER_DOWN, ref ret2);
                ...
                case STEP.STOPPER_DOWN_CHECK:
                    if (timeWait[(int)TIMER.TIMEOUT].Elapsed > Machine.param.Time(ParameterDefine.TIME.CYLINDER_TIME_OUT))
                    {
                        SetError(ECODE.TIMEOUT_JIG_INCARRIER_STOPPER_DOWN);
                        break;
                    }
                    Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_STOPPER_UP, ref ret1);
                    Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_STOPPER_DOWN, ref ret2);
        ...
        private CONVSTATUS ProductExistCheck()
        {
            ...
            Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_DETECT_IN, ref existIn);
            Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_DETECT_OUT, ref existOut);
            Machine.IO.GetIn((int)DI.JIG_IN_CARRIER_STOPPER_DOWN, ref stopperDown);

            if (existIn == 0 && existOut == 1 && stopperDown == 0) return CONVSTATUS.EXIST;
